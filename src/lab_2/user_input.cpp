...
void handleUserInput(GLFWwindow* window) {
  ...
  if (glfwGetKey(window, GLFW_KEY_1)) {
    ...
    glfwSetWindowTitle(window, "Rot. Pos X!\n");
  }
  // Press `2` - Rotate negative x
  if (glfwGetKey(window, GLFW_KEY_2)) {
    rotX -= DEGREE_INCREMENT;
    glfwSetWindowTitle(window, "Rot. Neg X!\n");
  }
  // Press `3` - Rotate positive y
  if (glfwGetKey(window, GLFW_KEY_3)) {
    rotY += DEGREE_INCREMENT;
    glfwSetWindowTitle(window, "Rot. Pos Y!\n");
  }
  // Press `4` - Rotate negative y
  if (glfwGetKey(window, GLFW_KEY_4)) {
    rotY -= DEGREE_INCREMENT;
    glfwSetWindowTitle(window, "Rot. Neg Y!\n");
  }
  // Press `5` - Rotate positive z
  if (glfwGetKey(window, GLFW_KEY_5)) {
    rotZ += DEGREE_INCREMENT;
    glfwSetWindowTitle(window, "Rot. Pos Z!\n");
  }
  // Press `6` - Rotate negative z
  if (glfwGetKey(window, GLFW_KEY_6)) {
    rotZ -= DEGREE_INCREMENT;
    glfwSetWindowTitle(window, "Rot. Neg Z!\n");
  }
  ...
  if (glfwGetKey(window, GLFW_KEY_D)) {
    ...
    glfwSetWindowTitle(window, "Transl. Pos X!\n");
  }
  // Press `A` - Translate negative x
  if (glfwGetKey(window, GLFW_KEY_A)) {
    transX -= TRANSLATE_INCREMENT;
    glfwSetWindowTitle(window, "Transl. Neg X!\n");
  }
  // Press `W` - Translate positive y
  if (glfwGetKey(window, GLFW_KEY_W)) {
    transY += TRANSLATE_INCREMENT;
    glfwSetWindowTitle(window, "Transl. Pos Y!\n");
  }
  // Press `S` - Translate negative y
  if (glfwGetKey(window, GLFW_KEY_S)) {
    transY -= TRANSLATE_INCREMENT;
    glfwSetWindowTitle(window, "Transl. Neg Y!\n");
  }
  // Press `Q` - Translate positive z
  if (glfwGetKey(window, GLFW_KEY_Q)) {
    transZ += TRANSLATE_INCREMENT;
    glfwSetWindowTitle(window, "Transl. Pos Z!\n");
  }
  // Press `E` - Translate negative z
  if (glfwGetKey(window, GLFW_KEY_E)) {
    transZ -= TRANSLATE_INCREMENT;
    glfwSetWindowTitle(window, "Transl. Neg Z!\n");
  }
  ...
  if (glfwGetKey(window, GLFW_KEY_MINUS)) {
    ...
    glfwSetWindowTitle(window, "Scale Uniform Neg!\n");
  }
  ...
  if (glfwGetKey(window, GLFW_KEY_EQUAL)) {
    ...
    scaleY += SCALE_INCREMENT;
    scaleZ += SCALE_INCREMENT;
    glfwSetWindowTitle(window, "Scale Uniform Plus!\n");
  }
  ...
  if (glfwGetKey(window, GLFW_KEY_Z)) {
    scaleX -= SCALE_INCREMENT;
    glfwSetWindowTitle(window, "Scale Non-Uniform Neg X!\n");
  }
  // Press `X` - Scale Non-Uniform Positive X
  if (glfwGetKey(window, GLFW_KEY_X)) {
    scaleX += SCALE_INCREMENT;
    glfwSetWindowTitle(window, "Scale Non-Uniform Pos X!\n");
  }
  // Press `C` - Scale Non-Uniform Negative Y
  if (glfwGetKey(window, GLFW_KEY_C)) {
    scaleY -= SCALE_INCREMENT;
    glfwSetWindowTitle(window, "Scale Non-Uniform Neg Y!\n");
  }
  // Press `V` - Scale Non-Uniform Positive Y
  if (glfwGetKey(window, GLFW_KEY_V)) {
    scaleY += SCALE_INCREMENT;
    glfwSetWindowTitle(window, "Scale Non-Uniform Pos Y!\n");
  }
  // Press `B` - Scale Non-Uniform Negative Z
  if (glfwGetKey(window, GLFW_KEY_B)) {
    scaleZ -= SCALE_INCREMENT;
    glfwSetWindowTitle(window, "Scale Non-Uniform Neg Z!\n");
  }
  // Press `N` - Scale Non-Uniform Positive Z
  if (glfwGetKey(window, GLFW_KEY_N)) {
    scaleZ -= SCALE_INCREMENT;
    glfwSetWindowTitle(window, "Scale Non-Uniform Pos Z!\n");
  }
}
