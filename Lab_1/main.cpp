...
// be using text files for shaders in future)
// Note: Input to this shader is the vertex positions that we specified for the
// triangle.
// Note: gl_Position is a special built-in variable that is supposed to contain
...
static void AddShader(GLuint ShaderProgram, const char* pShaderText,
  ...
  if (!success) {
    ...
    fprintf(
      stderr, "Error compiling shader type %d: '%s'\n", ShaderType, InfoLog
    );
    exit(1);
  }
...
GLuint generateObjectBuffer(GLfloat vertices[], GLfloat colors[]) {
  ...
  // execute commands on that target.
...
void display() {
	...
	// NB: Make the call to draw the geometry in the currently activated vertex
  // buffer. This is where the GPU starts to work!
